...
public class Wave implements Comparable<Wave> {
    ...
    //波浪线条颜色
    public List<Integer> waveColor = new ArrayList<>();
    ...
    public Wave(float offsetX, int waveHeight, int waveSpeed, int waveColor, int waveStrokeWidth) {
        ...
        this.waveColor.add(waveColor);
        this.waveStrokeWidth = waveStrokeWidth;
    }

    public Wave(float offsetX, int waveHeight, int waveSpeed, List<Integer> colorList, int waveStrokeWidth) {
        if (colorList == null || colorList.size() == 0) {
            waveColor.add(Color.BLUE);
        } else {
            waveColor.addAll(colorList);
        }
        //对边界值进行处理
        offsetX = Math.max(0, Math.min(1, offsetX));
        this.offsetX = offsetX;
